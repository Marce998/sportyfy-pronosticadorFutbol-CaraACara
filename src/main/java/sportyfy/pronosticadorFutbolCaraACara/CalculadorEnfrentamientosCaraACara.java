...
public class CalculadorEnfrentamientosCaraACara {
    ...
    public Map<Partido, Resultado> enfrentamientosEntreSi(Equipo local, Equipo visitante,
            Map<Partido, Resultado> partidos) {
        return partidos.entrySet().stream()
                .filter(entrada -> (entrada.getKey().getLocal().getNombre().equals(local.getNombre())
                        && entrada.getKey().getVisitante().getNombre().equals(visitante.getNombre()))
                        || entrada.getKey().getLocal().getNombre().equals(visitante.getNombre())
                                && entrada.getKey().getVisitante().getNombre().equals(local.getNombre()))
    ...
    public int cantidadPartidosGanadosEquipo(Equipo equipo, Map<Partido, Resultado> partidos) {
        return (int) partidos.entrySet().stream()
                .filter(entrada -> entrada.getKey().getGanador(entrada.getValue())
                        .map(ganador -> ganador.getNombre().equals(equipo.getNombre())).orElse(false))
                .count();
    }
